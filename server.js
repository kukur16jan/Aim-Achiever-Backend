...
app.use(cors({origin: 'https://aim-achiver-lovat.vercel.app', credentials: true,methods: ['GET', 'POST', 'PUT', 'DELETE'],
...
const fetchQuizForMicrotask = async (microtaskTitle) => {
  try {
    ...
    const res = await fetch(`${process.env.BACKEND_URL}/api/gemini-quiz`, {
      method: 'POST',
...
app.post('/api/moods/gemini', async (req, res) => {
  ...
  try {
    // Call Gemini
    const fetch = (...args) => import('node-fetch').then(({default: fetch}) => fetch(...args));
    const geminiRes = await fetch(`${process.env.BACKEND_URL}/api/gemini-mood`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
